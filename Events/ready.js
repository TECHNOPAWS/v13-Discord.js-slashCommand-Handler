...
module.exports = {
  ...
  async execute({ client }) {
    ...
    const slashCommand = client.slashCommands.map((m) => m.data.toJSON());

    // Check if the slashCommands must be registered globally or not.
    if (isGlobal) {

      // Register the command with a json request
      const globalRegister = await fetch(`https://discord.com/api/v9/applications/${client.user.id}/commands`, {
        method: `PUT`,
        headers: {
      ...
      const guildRegister = await fetch(`https://discord.com/api/v9/applications/${client.user.id}/guilds/${guildId}/commands`, {
        method: `PUT`,
        headers: {
          authorization: `Bot ${client.token}`,
          'Content-type': `application/json`,
